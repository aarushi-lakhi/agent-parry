...
class PolicyBundle:
    """A policy engine plus the inspectors derived from the same settings block."""
...
def build_policy_bundle(policy_path: str | Path) -> PolicyBundle:
    """Load a policy file into an engine and its settings-derived inspectors.
...
class StdioMcpProxy:
    ...
    def swap_policy(self, bundle: PolicyBundle) -> None:
        """Install an already-built policy bundle, engine and inspectors together.
...
class PolicyFileWatcher:
    ...
    async def poll_once(self) -> bool:
        ...
        self._stamp = stamp
        try:
            bundle = await asyncio.to_thread(build_policy_bundle, self._path)
        except Exception as exc:
            logger.warning(
...
async def _run_proxy(argv: list[str]) -> int:
    ...
    if parsed.reload_on_change:
        watcher = PolicyFileWatcher(proxy, policy_path, interval=parsed.reload_interval)
