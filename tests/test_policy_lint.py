...
def test_generator_no_longer_emits_an_empty_pattern_for_a_blank_argument(tmp_path: Path) -> None:
    """The generator refuses the match-all rule the linter would otherwise flag high."""
    ...
    lint = lint_rules(tmp_path, [pattern_rule("autogen_pi-blank", [""], field="body")])
...
def test_render_report_includes_the_clean_corpus_and_summary() -> None:
    ...
    assert f"over-block rate: 0/{report.benign_total} = 0.0%" in text
